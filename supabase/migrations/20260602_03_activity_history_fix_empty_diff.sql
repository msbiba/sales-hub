-- Migration 3/3: Fix für log_changes()
--
-- Bug: `is distinct from` wertet NULL vs '' als Unterschied. Forms senden
-- häufig '' für leere Felder, alte DB-Zeilen haben aber NULL → unnötige
-- History-Einträge wie 'telefon: NULL → ""' (UI: „—" → „—").
--
-- Fix: Vergleich via coalesce(x, '') <> coalesce(y, '') für alle text-
-- Felder, damit NULL und Leerstring als gleichwertig gelten.
-- Numerische und Datums-Felder verwenden weiter `is distinct from`.
...
begin
  ...
  if tg_op = 'UPDATE' then
    ...
    if tg_table_name = 'kunden' then
      ...
      -- Text-Felder: NULL und '' gelten als gleichwertig
      ...
      -- Status: Enum oder text, NULL <> '' Edge Case eher nicht relevant,
      -- aber via coalesce konsistent halten
      ...
      -- Numerische / Datums-Felder: NULL-Semantik weniger relevant,
      -- `is distinct from` ausreichend
      ...
      -- Sonderbehandlung: bearbeiter_id → Email-Resolution
  ...
  -- DELETE: CASCADE räumt activity_history selbst auf
  return old;
end;
$$;

-- Optionales Cleanup: bereits angefallene Phantom-Einträge entfernen,
-- bei denen old_value und new_value beide leer (NULL oder '') sind.
-- Vor Ausführung prüfen — bei Bedarf einzelne Reihen via SELECT inspizieren:
